#!/usr/bin/env bash
# =============================================================================
# FcstLabPro v0302 — Google Cloud Run Job 部署脚本
# 每天00 (UTC 00:00)北京时间 08: 运行
#
# 模型实验报告：experiments/weekly/v0302_label_experiment/final_summary.md
...
#   ./deploy/deploy_v0302.sh              # 完整流程
#   ./deploy/deploy_v0302.sh build        # 仅构建镜像
#   ./deploy/deploy_v0302.sh deploy       # 仅部署 Job
#   ./deploy/deploy_v0302.sh scheduler    # 仅设置定时
# =============================================================================
set -euo pipefail
...
IMAGE_NAME="fcstlabpro-v0302"
IMAGE_TAG="latest"
JOB_NAME="daily-btc-signal-v0302"
SCHEDULER_NAME="daily-btc-signal-v0302-trigger"
MEMORY="16Gi"
CPU="4"
...
# ─────────────────────────────────────────────────────────────
echo ""
echo "=== FcstLabPro v0302 部署脚本 ==="
echo "  策略: dip_recovery + Trigger A + Position sizing (linear)"
echo "  关键改进: Sharpe 0.9472, MaxDD 3.75%"
echo "  Label: dip_recovery (dip >5%, recovery >3%)"
echo "  触发: 等待 dip ≥5% 再入场"
echo "  退出: 止盈 +4%，止损 -3%，时间止损 14 天"
echo "  Position sizing: size = 2 × (prob - 0.5)"
echo "  邮件通知: 启用"
echo "  LLM分析: 启用"
echo ""
echo "  注意: 与 v0301 并行部署，互不影响"
echo ""
echo "=== Step 0: 前置检查 ==="
echo "  项目: ${PROJECT_ID}"
echo "  区域: ${REGION}"
