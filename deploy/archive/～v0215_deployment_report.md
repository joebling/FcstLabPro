...
| **Orion-BiX v27** | **0.1307** | 0.0000 | Bull 突破，Bear 失败 |
...
- **PnL 表现优于 GBDT v15**: 年化收益 +26.63% vs -11%
...
### 2026-02-16: flow.py 内存优化

**问题**: 原 flow.py 使用 `df[col] = ...` 逐列插入，导致 DataFrame 碎片化，在 Cloud Run 16Gi 环境下 OOM (exit code 137)。

**修复**: 改用 dict + pd.concat 模式，避免循环中反复添加列。
...
new_cols = {}
for w in [5, 10, 20]:
    new_cols[f"trades_sma_{w}"] = trades.rolling(w).mean()
df = pd.concat([df, pd.DataFrame(new_cols, index=df.index)], axis=1)
```

**验证**: 修改后本地运行峰值内存 ~12.8GB，低于 Cloud Run 16Gi 限制。
