...
1.  **命名规则**: `weekly_T{n}_X{m}_{suffix}`
    *   **`weekly`**: 数据频率为周线（每行数据代表一周）。
    *   **`T{n}`**: **预测窗口 (Time Horizon)** 为 `n` 周。例如 `T4` 表示关注未来 4 周的走势。
    *   **`X{m}`**: **涨幅阈值 (Threshold)** 为 `m`%。例如 `X5` 表示 5% 的涨幅标准。
    *   **后缀**:
        *   `refined_top25`: 仅使用经筛选后特征重要性最高的 25 个特征。
        *   `conservative`: 使用“保守策略”参数（低学习率、深树），旨在防止过拟合。
        *   `enhanced`: 包含链上数据等额外扩展特征。
