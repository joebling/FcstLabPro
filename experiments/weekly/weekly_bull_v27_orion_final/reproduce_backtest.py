...
def run_backtest_for_model(model_name, y_true, y_pred, prices):
    ...
    if n_samples > n_prices - 1:
        y_true = y_true[:n_prices - 1]
        y_pred = y_pred[:n_prices - 1]
        print(f"截断到: {len(y_true)} 个样本")
...
def main():
    ...
    models = [
        ...
        {
            'name': 'Orion-BiX v27 (Bull)',
            'path': 'experiments/weekly/weekly_bull_v27_orion_final/predictions.csv'
        }
    ]
