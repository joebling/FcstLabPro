...
### 9.1 成本敏感性 (Regime=-10%, TP=4%)

| 成本 | Return | Sharpe | MaxDD | PF |
|------|--------|--------|-------|-----|
| 0.1% | +64.29% | 0.76 | -21.40% | 1.28 |
| 0.2% | +54.55% | 0.68 | -22.18% | 1.27 |
| 0.3% | +45.38% | 0.60 | -22.96% | 1.25 |

✅ **成本稳健**：即使 0.3% 单边成本，仍有 +45% 收益、PF>1.25。

### 9.2 Regime 阈值敏感性 (Cost=0.1%, TP=4%)

| Regime | Return | Sharpe | MaxDD | Exposure |
|--------|--------|--------|-------|----------|
| -8% | +31.95% | 0.50 | -19.72% | 20.9% |
| **-10%** | **+64.29%** | **0.76** | -21.40% | 23.3% |
| -12% | +41.11% | 0.55 | -20.85% | 25.9% |

✅ **-10% 是最优阈值**，与 E1 生产配置一致。-8% 的 MaxDD 可压到 -19.7%，但收益减半。

### 9.3 止盈阈值敏感性 (Regime=-10%, Cost=0.1%)

| TP | Return | Sharpe | MaxDD | PF |
|----|--------|--------|-------|-----|
| 3% | +49.68% | 0.65 | -21.16% | 1.26 |
| **4%** | **+64.29%** | **0.76** | -21.40% | **1.28** |
| 5% | +56.64% | 0.69 | -23.02% | 1.25 |

✅ **4% 止盈最优**，正好与标签的 X=4% 一致。

### 9.4 综合结论

E8 的默认配置 (Regime=-10%, Cost=0.1%, TP=4%) 就是全网格最优解，无需调参。

---

## 十、Paper Trading 设置

Paper trading 脚本已就绪，每日运行：

```bash
# 每日运行 (建议北京时间 08:00)
python3.10 scripts/paper_trading_e1_vs_e8.py --download --save
```

信号保存在 `data/live/paper_trading/signal_YYYY-MM-DD.json`。

**Paper Trading 监控要点** (4~8 周)：
- [ ] 每笔交易 MFE/MAE
- [ ] E1 vs E8 信号一致性率
- [ ] E8 独有信号的净值贡献
- [ ] 连续亏损次数监控 (燔断线: 3次连亏)
