...
print("=" * 80)
print("exp14_recent_year: 详细交易记录")
print("=" * 80)
...
if len(trades_df) > 0:
    ...
    print(f"\n   全部交易: {OUTPUT_DIR / 'all_trades.csv'}")
    
    if len(recent_trades) > 0:
        recent_trades.to_csv(OUTPUT_DIR / "recent_year_trades.csv", index=False)
        print(f"   最近一年: {OUTPUT_DIR / 'recent_year_trades.csv'}")

print("\n8. 生成报告...")

with open(OUTPUT_DIR / "results_trades.md", "w", encoding="utf-8") as f:
    f.write("# exp14_recent_year: 详细交易记录报告\n\n")
...
print(f"\n   报告已保存: {OUTPUT_DIR / 'results_trades.md'}")
print("\n" + "=" * 80)
print("完成!")
print("=" * 80)
