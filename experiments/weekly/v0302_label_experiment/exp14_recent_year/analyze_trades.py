...
print("=" * 80)
print("exp14_recent_year: 交易记录分析")
print("=" * 80)
...
if len(trades_df) > 0:
    trades_df.to_csv(OUTPUT_DIR / "all_trades_valid.csv", index=False)
    print(f"\n   全部交易: {OUTPUT_DIR / 'all_trades_valid.csv'}")
    
    if len(recent_trades) > 0:
        recent_trades.to_csv(OUTPUT_DIR / "recent_year_trades_valid.csv", index=False)
        print(f"   最近一年: {OUTPUT_DIR / 'recent_year_trades_valid.csv'}")

print("\n8. 生成报告...")

with open(OUTPUT_DIR / "results_final.md", "w", encoding="utf-8") as f:
    f.write("# exp14_recent_year: 最终交易记录报告\n\n")
...
print(f"\n   报告已保存: {OUTPUT_DIR / 'results_final.md'}")
print("\n" + "=" * 80)
print("完成!")
print("=" * 80)
