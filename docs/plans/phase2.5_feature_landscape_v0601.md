...
### 9.1 ⚠️ 认知校正: “链上指标都没用” 是错误结论

截至 2026-06-02, 链上指标族 (LTH/STH / PUELL / SOPR / CDD / FUNDING) 在本项目全部失败。
**但不能归因为 “链上数据无用”** — 这是 over-generalization (拿 3-4 个负样本否定一整类因子)。

**严谨结论**: 这些指标在 **日频 (1d) + directional 标签 + T=21 + 单资产 BTC + LightGBM**
这个狭窄条件下无正交增量。三个譁据说明真凶是**任务设定**而非“链上”本身:

1. **失败模式不一致** → 不是同一共性在作祟: PUELL=冗余(共线), SOPR/CDD=信号弱, LTH/STH=尺度错配。
2. **时间尺度错配 (最可能真因)**: 链上指标天生是周期级慢变量 (有效周期数月〜一个减半周期),
   拿去预测 21 天方向 = 用年度 GDP 预测明天股价。
3. **测试方式偏窄**: 全是 directional 分类, 没测过链上指标更擅长的 **regime 识别** /
   **长 horizon (周/月频)** / **极值择时** (SOPR<1 拄底, Puell>4 逃顶)。

**机构用法 (未测)**: regime gating (MVRV-Z 判牛熊) / 月度配置 / 二元极值择时。
这恰好是 Phase 3 (方向 3: regime/多时间框架) 给链上指标**平反的正确舞台**,
而非在日频 directional 上反复撑死。

> **可复用结论**: “某因子在条件 X 下无效” ≠ “某因子无效”。关闭的是 *(指标 × 任务) 组合*,
> 不是指标本身。换任务设定 (方向 3) 时, 链上指标应重新评估, 不受本表判决约束。
